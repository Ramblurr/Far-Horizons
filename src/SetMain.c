...
int species_number, species_index, data_was_modified, num_arguments, abbr_type, abbr_index, sub_light, tonnage, next_arg;
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 2 && isdigit(argv[1][0]) && !isdigit(argv[2][0])) {
        set_species();
    } else if (argc > 4 && isdigit(argv[1][0]) && isdigit(argv[2][0]) && isdigit(argv[3][0]) && !isdigit(argv[4][0])) {
        set_star();
    } else if (argc > 5 && isdigit(argv[1][0]) && isdigit(argv[2][0]) && isdigit(argv[3][0]) && isdigit(argv[4][0])) {
        set_planet();
    }
    fprintf(stderr,
...
void set_species(void) {
    ...
    if (abbr_type == PLANET_ID) {
        ...
        printf("SP %s, %s tech level = %d", species->name, tech_name[abbr_index], species->tech_level[abbr_index]);
        ...
        printf("SP %s has %ld economic units", species->name, species->econ_units);
        ...
        printf("SP %s hp_original_base = %ld", species->name, species->hp_original_base);
        if (next_arg < num_arguments) {
            if (isdigit(argument[next_arg][0])) {
                n = atol(argument[next_arg++]);
                printf(", changed to %ld", n);
...
void set_nampla(void) {
    ...
    switch (abbr_type) {
        case ITEM_CLASS:
            ...
            printf("SP %s, PL %s, %ld %ss", species->name, nampla->name, nampla->item_quantity[abbr_index],
                   item_name[abbr_index]);
        ...
        default:
            ...
            if (strcasecmp(argument[next_arg], "x") == 0) {
                printf("SP %s, PL %s, x = %d", species->name, nampla->name, nampla->x);
                ++next_arg;
                break;
            } else if (strcasecmp(argument[next_arg], "y") == 0) {
                printf("SP %s, PL %s, y = %d", species->name, nampla->name, nampla->y);
                ++next_arg;
                break;
            } else if (strcasecmp(argument[next_arg], "z") == 0) {
                printf("SP %s, PL %s, z = %d", species->name, nampla->name, nampla->z);
                ++next_arg;
                break;
            } else if (strcasecmp(argument[next_arg], "pn") == 0) {
                printf("SP %s, PL %s, pn = %d", species->name, nampla->name, nampla->pn);
                ++next_arg;
                break;
            } else if (strcasecmp(argument[next_arg], "sy") == 0) {
                printf("SP %s, PL %s, %d shipyards", species->name, nampla->name, nampla->shipyards);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, PL %s, mining base = %d", species->name, nampla->name, nampla->mi_base);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, PL %s, manufacturing base = %d", species->name, nampla->name, nampla->ma_base);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, PL %s, available population = %d", species->name, nampla->name, nampla->pop_units);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
...
void set_ship(void) {
    ...
    switch (abbr_type) {
        case ITEM_CLASS:
            printf("SP %s, %s, %ld %ss", species->name, ship_name(ship), ship->item_quantity[abbr_index],
                   item_name[abbr_index]);
            if (++next_arg < num_arguments) {
                if (isdigit(argument[next_arg][0])) {
                    n = atol(argument[next_arg++]);
        ...
        default:
            if (strcasecmp(argument[next_arg], "x") == 0) {
                printf("SP %s, %s, x = %d", species->name, ship_name(ship), ship->x);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, %s, y = %d", species->name, ship_name(ship), ship->y);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, %s, z = %d", species->name, ship_name(ship), ship->z);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, %s, pn = %d", species->name, ship_name(ship), ship->pn);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, %s, loading_point = %d", species->name, ship_name(ship), ship->loading_point);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, %s, unloading_point = %d", species->name, ship_name(ship), ship->unloading_point);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
                ...
                printf("SP %s, %s, age = %d turns", species->name, ship_name(ship), ship->age);
                if (++next_arg < num_arguments) {
                    if (isdigit(argument[next_arg][0])) {
                        n = atol(argument[next_arg++]);
                        printf(", changed to %ld", n);
...
void set_star(void) {
    ...
    if (strcasecmp(argument[next_arg], "np") == 0) {
        printf("For star at %d %d %d, num planets = %d", x, y, z, star->num_planets);
        ++next_arg;
    } else {
        fprintf(stderr, "\n\tInvalid argument: '%s'!\n\n", argument[next_arg]);
        exit(-1);
...
void set_planet(void) {
    ...
    if (strcasecmp(argument[next_arg], "md") == 0) {
        printf("For planet #%d at %d %d %d, mining difficulty = %d", pn, x, y, z, planet->mining_difficulty);
        ...
        printf("For planet #%d at %d %d %d, economic efficiency = %d", pn, x, y, z, planet->econ_efficiency);
        if (++next_arg < num_arguments) {
            if (isdigit(argument[next_arg][0])) {
                n = atol(argument[next_arg++]);
                printf(", changed to %ld", n);
        ...
        printf("For planet #%d at %d %d %d, pressure class = %d", pn, x, y, z, planet->pressure_class);
        if (++next_arg < num_arguments) {
            if (isdigit(argument[next_arg][0])) {
                n = atol(argument[next_arg++]);
                printf(", changed to %ld", n);
        ...
        printf("For planet #%d at %d %d %d, temperature class = %d", pn, x, y, z, planet->temperature_class);
        if (++next_arg < num_arguments) {
            if (isdigit(argument[next_arg][0])) {
                n = atol(argument[next_arg++]);
                printf(", changed to %ld", n);
        ...
        printf("For planet #%d at %d %d %d, gravity = %d", pn, x, y, z, planet->gravity);
        if (++next_arg < num_arguments) {
            if (isdigit(argument[next_arg][0])) {
                n = atol(argument[next_arg++]);
                printf(", changed to %ld", n);
...
char *ship_name(struct ship_data *ship) {
    ...
    if (ship_is_distorted) {
        if (ship->class == TR) {
            sprintf(full_ship_id, "%s%d ???", ship_abbr[ship->class], ship->tonnage);
        ...
        sprintf(full_ship_id, "%s%d%s %s", ship_abbr[ship->class], ship->tonnage, ship_type[ship->type], ship->name);
    } else {
